...
따라서 이 비교의 중심은 backbone이 국소적으로 단백질처럼 보이는지가 아니라, sequence가 다시 찾아갈 수 있는 자연 구조 근처의 영역에 남아 있는지에 있다.
...
- 다만 평가는 CA trace 기반 ProteinMPNN과 ESMFold 조합에 한정된다.
...
<figure class="media-figure media-figure--wide-visual">
  ...
  <figcaption><strong>Figure 1.</strong> 구조 단서 수준별 moderate 통과율을 압축해 비교했다. 조건별 n이 같지 않으므로, 막대는 전체 순위가 아니라 구조 단서 수준별 판정 차이를 보여주는 요약이다. Strict와 mean TM은 Table 4에서 함께 정리한다.</figcaption>
</figure>
...
이 비교는 자연 구조 core를 고정했을 때 편집 방식별 designability 보존 범위를 확인한다. 같은 core를 유지해도, 주변 segment를 부드럽고 제한된 범위에서 바꾼 경우에만 designability가 남았다.
...
부드러운 제한형 segment 편집은 6/8 moderate, 5/8 strict였지만, 양 끝점 직선 보간은 0/8, fragment 삽입은 1/8 moderate에 그쳤다. 즉 core 좌표만 남기는 것으로는 부족했고, edited segment가 core와 무리 없이 이어지며 자연 구조 근처를 크게 벗어나지 않는 편집 형태가 필요했다. 표본이 작으므로 일반화에는 주의가 필요하며, 재검증과 소표본 보조 분석은 Appendix에 둔다.
...
이 범위 안에서는 designability가 backbone을 자유롭게 새로 만드는 능력보다, 설계된 sequence가 다시 찾아갈 수 있는 자연 구조 근방에서 얼마나 덜 벗어나는지에 더 민감하게 반응했다.
...
따라서 이 설정의 designability는 국소 기하를 맞추는 문제라기보다, 설계된 sequence가 다시 찾아갈 수 있는 자연 단백질 구조 근처의 영역에 머무는 문제에 가깝다.
...
이 결론을 모델 설계 방향으로 옮기면 backbone 전체를 새로 생성하는 diffusion generator보다, 자연 구조 core를 유지하고 editable segment의 좌표 변화만 제한적으로 예측하는 local editor가 더 적절한 출발점이 된다. Figure 2는 평가에서 사용한 모델 구조가 아니라, Table 5의 관찰에서 도출한 후속 local editor 설계안이다.
