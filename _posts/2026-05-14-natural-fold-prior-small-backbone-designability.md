...
excerpt: "자연 구조 4개와 구조당 2개 segment를 반복 비교한 pilot에서 국소 편집의 구조 복원 결과와 CA-only 평가의 한계를 정리한다."
description: "자연 단백질 4개 구조의 8개 segment 패널에서 부드러운 편집은 6/8 moderate, 5/8 strict를 보였다. paired 반복 구조를 고려해 기술적 관찰로 제한한 pilot이다."
...
자연 단백질의 core를 유지하면서 일부 segment를 바꾸면 설계한 sequence가 편집된 backbone과 비슷한 구조로 돌아오는지 살펴봤다. 중심 비교는 자연 구조 4개와 구조당 16-residue segment 2개로 만든 8개 조건을 세 편집 방식에 반복 적용한 pilot이다.

Dauparas et al.의 CA-only ProteinMPNN으로 sequence를 설계하고, Lin et al.의 ESMFold로 구조를 예측했다 <a class="citation-ref" href="#ref-proteinmpnn" aria-label="Reference 1">[1]</a><a class="citation-ref" href="#ref-esmfold" aria-label="Reference 2">[2]</a>. 이 글의 designability는 두 모델을 거친 구조 복원 기준이며 실험적 안정성이나 기능을 뜻하지 않는다.

부드러운 제한형 편집은 6/8 moderate, 5/8 strict를 기록했다. 같은 패널의 직선 보간은 0/8, fragment 삽입은 1/8 moderate였다. 이 차이는 작은 자연 구조 패널의 관찰이다. 독립적인 8개 단백질 비교나 학습된 편집 모델의 성능으로 해석하지 않는다.

국소 기하·topology 생성 결과는 탐색 배경으로 함께 보존한다. 조건별 후보와 모델이 달라 이들 사이의 차이로 designability에 필요한 구조 정보의 최소 수준을 식별할 수는 없다.

> **자연 단백질 구조**는 PDB/CATH 같은 자연 단백질 구조 데이터에서 관찰되는 backbone 형태와 topology를 가리킨다. 여기에는 자연 단백질의 전체 backbone을 그대로 쓰는 조건뿐 아니라, 실제 자연 단백질 topology, 고정된 자연 구조 core, 자연 구조 근처에서의 제한된 segment 편집처럼 sequence가 다시 접힐 수 있는 구조 근방의 정보를 생성/편집 과정에 넣는 조건이 포함된다.

<aside class="research-question" aria-label="평가 초점">
  <p class="research-question__label">Experiment Focus</p>
  <p>같은 자연 구조와 segment 패널에서 편집 방식에 따라 ProteinMPNN + ESMFold 구조 복원 기준을 통과한 후보 수가 어떻게 달라지는지 살펴본다.</p>
</aside>
...
- 중심 질문은 자연 구조 4개 × segment 2개의 반복 패널에서 편집 방식별 구조 복원이 어떻게 달라지는지다.
...
- 현재 결과는 diffusion 기반 전체 생성보다, 자연 구조 core 근처의 editable segment를 제한적으로 바꾸는 local editor를 후속 설계안으로 검토하게 한다. 아직 학습 모델 성능으로 검증된 것은 아니다.
- 평가는 CA trace 기반 ProteinMPNN과 ESMFold 조합에 한정된다. 같은 구조·segment의 paired 반복이므로 독립 2군 검정이나 8개 독립 표본으로 일반화하지 않는다.
...
중심 비교는 자연 구조 4개와 구조당 segment 2개에서 편집 방식별 구조 복원을 살펴보는 pilot이다. 별도 생성 조건은 탐색 배경이며, RFdiffusion, Chroma, FrameDiff 같은 대형 backbone 생성 모델과 규모를 맞춘 비교는 포함하지 않았다.
  ...
  <div class="table-shell">
    <table class="comparison-table">
      ...
      <tbody>
        <tr>
          <td>비교 질문</td>
          <td>같은 자연 구조 core 주변의 편집 방식별 구조 복원</td>
        </tr>
        <tr>
          <td>구조 단서</td>
...
먼저 Table 3은 평가에서 시도한 모델 계열과 구조 표현을 결과 해석용으로 요약한다.
    ...
    <table class="comparison-table">
      ...
      <tbody>
        ...
        <tr>
          ...
          <td>자연 구조에 가깝게 보정</td>
          <td>보정만으로 designability 기준 통과로 이어지지 않음</td>
        </tr>
      </tbody>
    </table>
  </div>
...
<figure class="media-figure media-figure--wide-visual">
  ...
  <figcaption><strong>Figure 1.</strong> 구조 단서 수준별 moderate 통과율을 압축해 비교했다. 조건별 표본 수뿐 아니라 후보 패널과 생성 방식도 달라, 통제된 prior 효과나 단일 순위로 해석할 수 없다. Strict와 mean TM은 Table 4에서 함께 정리한다.</figcaption>
</figure>

Figure 1은 moderate 통과율만 압축해 보여주고, Table 4는 같은 조건을 n, strict 기준, mean TM까지 포함해 자세히 정리한다. 표의 조건명은 Figure 1과 맞추고, 비슷한 조건끼리 구조 단서 그룹으로 묶었다.

<figure class="table-figure table-figure--comparison table-figure--metrics">
  <div class="table-shell">
    <table class="comparison-table metrics-table metrics-table--numeric-columns metrics-table--designability-ladder">
      <thead>
        <tr>
          <th>구조 단서 그룹</th>
          <th>요약 조건</th>
      ...
      <tbody>
        <tr>
          <td class="group-cell">국소 기하 정보</td>
          <td class="condition-cell">Local geometry<br><span class="table-note-inline">각도 기반 local diffusion</span></td>
        ...
        <tr>
          <td rowspan="2" class="group-cell">약한 topology 단서</td>
          <td class="condition-cell">Weak topology A<br><span class="table-note-inline">연속 거리 지도 + compactness/contact</span></td>
          <td class="align-right"><code>8</code></td>
          <td class="align-right"><code>0</code></td>
          <td class="align-right"><code>0</code></td>
        ...
        <tr>
          <td class="condition-cell">Weak topology B<br><span class="table-note-inline">residue 간 거리 구간 분류</span></td>
          <td class="align-right"><code>8</code></td>
          <td class="align-right"><code>0</code></td>
          <td class="align-right"><code>0</code></td>
        ...
        <tr>
          <td rowspan="2" class="group-cell">자연 단백질 구조 단서</td>
          <td class="condition-cell">Natural topology<br><span class="table-note-inline">실제 자연 단백질 topology 입력</span></td>
          <td class="align-right"><code>8</code></td>
        ...
        <tr>
          <td class="condition-cell">Natural backbone<br><span class="table-note-inline">자연 단백질 전체 backbone</span></td>
        ...
        <tr>
          <td rowspan="3" class="group-cell">자연 구조 core 편집</td>
          <td class="condition-cell">Smooth segment edit<br><span class="table-note-inline">core + 제한된 국소 편집</span></td>
          <td class="align-right"><code>8</code></td>
        ...
        <tr>
          <td class="condition-cell">Straight interpolation<br><span class="table-note-inline">core + 단순 국소 연결</span></td>
          <td class="align-right"><code>8</code></td>
          <td class="align-right"><code>0</code></td>
          <td class="align-right"><code>0</code></td>
        ...
        <tr>
          <td class="condition-cell">Fragment insertion<br><span class="table-note-inline">core + fragment 삽입</span></td>
          <td class="align-right"><code>8</code></td>
  ...
  <figcaption><strong>Table 4.</strong> Figure 1의 조건을 같은 이름으로 풀어 쓴 상세 표다. Moderate/strict는 n개 중 판정 기준을 통과한 개수이며, mean TM은 전체 구조 복원 정도를 읽기 위한 보조 지표다. 조건별 n과 역할이 다르므로 단일 순위표로 해석하지 않는다.</figcaption>
</figure>
...
이 비교는 자연 구조 core를 고정했을 때 편집 방식별 designability 보존 범위를 확인한다. 같은 core를 유지한 세 편집 방식 사이에서도 통과 후보 수가 달랐다.

Table 5는 같은 자연 구조 core 주변의 16-residue segment를 세 방식으로 바꾼 결과를 비교한다. 균형 비교 세트는 학습에서 분리해 둔 자연 구조 4개와 구조당 segment 2개를 사용했다.
...
<figure class="table-figure table-figure--comparison table-figure--metrics">
  ...
  <figcaption><strong>Table 5.</strong> 고정된 자연 구조 core 조건에서 세 가지 local 편집 방식을 비교했다. 비교의 초점은 core 유무가 아니라, 같은 core 주변 segment를 어떤 방식으로 바꿨을 때 구조 복원이 유지되는지다.</figcaption>
</figure>

부드러운 제한형 segment 편집은 6/8 moderate, 5/8 strict였지만, 양 끝점 직선 보간은 0/8, fragment 삽입은 1/8 moderate에 그쳤다. 이 패널에서는 core 좌표를 보존한 것만으로 모든 편집 방식에서 구조 복원이 유지되지는 않았다. 편집 형태가 이 차이의 원인인지는 분해되지 않았다. 같은 구조에서 두 segment를 반복했고 같은 segment를 방법 간 비교했으므로 8개 조건을 독립 표본으로 취급하지 않는다. 선별 후보 재검증과 추론 한계는 Appendix에 둔다.

자연 구조별 차이도 남아 있다. 균형 비교 세트의 4개 자연 구조(PDB: 1A43, 1UBQ, 2FK1, 2J6D) 중 3개에서는 부드러운 제한형 segment 편집이 기준 통과 후보를 만들었지만, 1개 구조에서는 만들지 못했다. 실패한 구조의 원인은 아직 분해하지 않았으므로, 이 차이는 후속 분석 대상으로 남긴다.
...
따라서 약한 topology 단서는 “접힌 것처럼 보이는 구조”를 만들 수는 있어도, ProteinMPNN이 설계한 sequence가 ESMFold에서 다시 찾아갈 수 있는 구조를 충분히 고정하지는 못한 것으로 본다.
...
Table 7에서는 국소 변형 조건의 moderate 통과 수가 큰 전체 변형보다 높았다. Strict 기준과 원형 대조까지 포함하면 단순한 단조 관계는 아니며, 자연 구조 근처의 여러 변형에서도 통과 후보가 남았다는 범위로 읽는다.

짧은 국소 편집에서는 원 구조 TM이 높고 moderate 통과 후보도 관찰됐다. 다만 편집 범위·형태와 원 구조와의 거리가 함께 달라졌으므로, 어느 요인이 구조 복원 차이를 만들었는지는 분리되지 않았다. 이 비교는 제한된 구조 패널에서 자연 구조 근처의 편집 가능성을 확인하는 근거로 둔다.

## 해석과 결론

가장 직접적인 관찰은 자연 구조 core 주변의 제한된 편집에서 구조 복원 후보가 남았다는 것이다. 4개 구조 × 2개 segment의 같은 패널에서 부드러운 편집의 통과 수가 높았지만 구조 수가 적고 반복 측정이므로 모집단 효과나 통계적 유의성으로 확대하지 않는다.

별도 탐색 조건의 0/8은 해당 생성 설정에서 통과 후보를 얻지 못했다는 뜻이다. 데이터·학습량·모델·후보 세트를 맞춘 단일요인 비교가 아니므로 자연 구조 prior가 필요조건이라는 증거는 아니다. 자연 backbone 대조도 10/20 moderate였다는 점은 이 평가 절차 자체가 모든 자연 구조를 통과시키지는 않음을 보여준다.

높은 원 구조 TM은 편집 범위가 자연 구조 근처였음을 설명한다. 전체 PDB/CATH 대비 구조적 새로움이나 de novo backbone 생성 능력은 확인하지 않았다.
...
- 학습 구조와의 유사도는 보조 점검했지만, 전체 PDB/CATH 대비 새로운 구조인지에 대한 평가는 아직 하지 않았다.
- 균형 비교 세트의 n=8은 자연 구조 4개에서 segment를 두 번씩 평가한 수다. 방법 간 pairing과 구조 내 의존성을 무시한 Fisher exact 검정·Wilson 구간은 추론 근거로 사용하지 않는다.
- 부드러운 제한형 segment 편집의 통과 수는 높았고, 양 끝점 직선 보간이나 fragment 삽입은 낮은 결과를 보였지만, 그 차이를 만드는 요인은 아직 충분히 분해하지 않았다.
...
이전 보조 분석은 같은 구조·segment를 반복한 패널에 독립 두 집단의 Fisher exact 검정과 독립 Bernoulli 표본의 Wilson 구간을 적용했다. 방법 간 pairing과 같은 자연 구조에 속한 두 segment의 의존성이 반영되지 않으므로 해당 p값과 구간을 비교 우위의 근거에서 제외했다. 원래 관찰한 통과 개수는 Table 5에 그대로 남긴다.

구조 ID × segment ID × 편집 방식별 pass/fail·연속 지표와 후보 생성·선택 규칙을 다시 확인할 원자료는 이번 개정에서 확인하지 못했다. 현재 집계로는 pairing과 구조별 의존성을 반영한 불확실성을 재계산하지 않는다. 더 많은 독립 자연 구조에서 같은 비교를 반복하는 일은 이 네 구조 pilot의 관찰을 확장하기 위한 후속 과제다.


- 구조별 차이: 균형 비교 세트의 자연 구조 PDB ID는 1A43, 1UBQ, 2FK1, 2J6D다. 부드러운 제한형 segment 편집은 1A43, 1UBQ, 2FK1에서 기준 통과 후보를 만들었고, 2J6D에서는 만들지 못했다. 2J6D 조건의 원인은 아직 분해하지 않았다.
- 새로움의 범위: nearest-train TM은 비교에 포함된 학습 구조와의 유사도를 나타내는 보조 지표이며, 전체 PDB/CATH 대비 새로운 구조인지에 대한 평가는 아직 포함하지 않았다.

</div>

## Appendix: 아직 검증하지 않은 local editor

후속 가설은 자연 구조 core를 유지하면서 editable segment의 좌표 변화만 제한적으로 예측하는 local editor다. 수동 편집 관찰이 학습 후에도 유지되는지, 전체 생성 모델보다 나은지는 아직 평가하지 않았다. Figure 2는 평가에서 사용한 모델 구조가 아니라, Table 5의 관찰에서 도출한 후속 local editor 설계안이다.
...
```
  ...
  <figcaption><strong>Figure 2.</strong> Table 5의 관찰에서 도출한 후속 local editor 설계안이다. 자연 구조 core를 둔 상태에서 editable segment에 적용할 bounded coordinate delta를 예측하는 1D coordinate editor로 정리했다. 아직 학습 및 성능 검증을 마친 모델은 아니다.</figcaption>
</figure>

ProteinMPNN + ESMFold 평가는 이 edited backbone에 별도로 적용한다. 남는 과제는 이 설계안을 실제 학습 가능한 국소 편집 모델로 옮겼을 때 같은 평가 기준을 통과하는 후보가 유지되는지 확인하는 것이다.
