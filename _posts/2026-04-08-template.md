...
last_modified_at: 2026-09-06 09:18:24 +0900
categories: [template]
...
이 문서는 공개 연구 노트의 재사용 가능한 표현 예시다. 새 글에는 필요한 컴포넌트만 선택한다.

## 작성 기준

- 문체·구조·용어: [연구 노트 편집 기준](../.agents/skills/blog-research-note/references/editorial.md)
- 표·caption·수식: [서식 기준](../.agents/skills/blog-research-note/references/formatting.md)
- 본문 Figure·대표 이미지·시각 QA: [시각 자료 기준](../.agents/skills/blog-research-note/references/visuals.md)
- 인용과 출처 확인: [인용 기준](../.agents/skills/blog-research-note/references/citations.md)
- Front matter·실험 출처·검색 노출: [SEO.md](../SEO.md)
- 실행·검증·발행: [WORKFLOW.md](../WORKFLOW.md)

기본 흐름은 문제 설정, 요약, 평가 설계, 결과, 해석과 한계, 필요한 Appendix, References/Experiment Resources, Citation이다. 모델 카드·탭·sample 비교를 모든 글에 넣을 필요는 없다.
...
대표 이미지와 hero의 역할·제작 기준은 위 시각 자료 기준을 따른다. 아래는 레이아웃별 옵션이다.

- PNG/JPG/WebP hero는 기본적으로 프레임을 채운다. 도식형 bitmap을 자르지 않으려면 `hero_fit: contain`을 사용한다.
- 가로형 flow SVG를 원본 `1200x430` 비율로 표시하려면 `hero_variant: flow-diagram`을 사용한다.
- 넓은 본문 SVG의 라벨을 유지하려면 `media-figure--wide-visual`로 모바일 가로 스크롤을 허용한다.

## 실험 경로 메타데이터 예시

실제 위치를 확인한 값만 기록한다. 필수 여부와 이전 경로 처리 기준은 [SEO.md](../SEO.md)의 실험 출처 섹션을 따른다.
...
```text
Ilho Ahn, "첫 글: 이 블로그를 어떻게 쓸지", Ilho’s Notes, Apr 2026.
```
