...
- 추세, trade-off, paired gain, axis 방향성이 중요한 결과는 SVG chart나 Plotly를 사용합니다.
- workflow, model/data flow, 결합 구조처럼 graph-like 구조는 Mermaid를 사용합니다.
- Mermaid가 node, label, annotation 때문에 빽빽해지면 SVG로 옮깁니다.
- hover, zoom, legend toggle이 실제 해석에 도움이 될 때만 Plotly를 사용합니다.
- prose나 기존 table만으로 충분히 읽히는 결과에는 장식용 figure를 추가하지 않습니다.

### Palette와 typography

- 기본 배경은 `#ffffff` 또는 아주 옅은 neutral surface를 사용합니다.
- 주 강조색은 기존 chart와 같은 `#3f41ff`를 우선하고, 비교 기준이나 비강조 값은 `#9aa1ad`, `#d9dde5`, `#5f6672` 계열로 낮춥니다.
...
- SVG text는 본문 chart 기준 11-12px보다 작게 만들지 않고, 제목/축/값/legend의 크기 차이를 과하게 벌리지 않습니다.
- 긴 method명은 SVG 안에 억지로 넣기보다 caption, 본문, 또는 table의 `<br>`와 `table-note-inline`로 분리합니다.
