...
- 참조 언어모델 NLL을 `nllnat`와 `nllctx`로 나누어 자동 선호 쌍 구성에 사용하는 절차를 구성했다.
...
대표 설정은 validation-stage 분석에서 먼저 고정한 뒤 held-out test에서 다시 집계했다. 선택 기준은 형식 통과율의 회복과 형식 통과 출력의 중앙값 `nllnat`/`nllctx` 유지 사이의 절충이다. 3 문구 기본 카드에서는 먼저 DPO 기준선과 형식 인지 합성 pair의 teacher를 고정한 뒤 `formatmix` 비율을 스윕했다. 본문에서 사용하는 `formatmix 0.25`는 형식 통과율을 최대화한 설정이 아니라, DPO가 만든 낮은 NLL 구간을 유지하면서 형식 손실을 상당 부분 복구한 대표 지점이다.

메타 슬롯 확장 카드에서는 DPO 기준선, 형식 전용 DPO 대조군, 여러 GRPO 보정 변형을 validation에서 비교했다. 본문 대표 GRPO 변형은 최고 형식 통과율 설정이 아니라, 비교한 GRPO 변형 중 낮은 `nllnat`를 유지하면서 형식 통과율을 크게 회복한 절충점이다. 세부 선택 근거와 후보별 수치는 Appendix Table 4와 Appendix Table 5에 정리한다.
...
<figure class="media-figure media-figure--wide-visual">
  <img src="/assets/images/posts/korean-ui-nll-proxy-alignment/format-nll-tradeoff.svg" alt="3 문구 기본 카드와 메타 슬롯 확장 카드에서 SFT, DPO, formatmix, GRPO의 형식 통과율과 중앙값 nllnat 절충을 비교한 산점도">
  <figcaption><strong>Figure 3.</strong> Held-out test 대표 결과의 형식-NLL 절충이다. x축은 형식 통과율, y축은 형식 통과 출력 부분집합의 중앙값 <code>nllnat</code>이다.</figcaption>
</figure>
    ...
    <table class="comparison-table metrics-table metrics-table--numeric-columns">
      ...
      <tbody>
        ...
        <tr>
          ...
          <td>본문 대표 설정. GRPO 계열 중 낮은 <code>nllnat</code>와 큰 형식 회복을 함께 보임</td>
        </tr>
        <tr>
          <td>고형식 GRPO 변형 A</td>
          ...
          <td>형식 통과율은 더 높지만 <code>nllnat</code> 회귀가 증가</td>
        </tr>
        <tr>
          <td>고형식 GRPO 변형 B</td>
          ...
          <td>최고 형식 통과율 설정이지만 <code>nllnat</code> 회귀가 가장 큼</td>
        </tr>
      </tbody>
    </table>
  </div>
