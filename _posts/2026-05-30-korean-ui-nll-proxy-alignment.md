...
last_modified_at: 2026-05-30 12:58:00 +0900
categories: ["LLM EVAL"]
...
description: "한국어 구조화 UI 문구 생성에서 NLL_nat와 NLL_ctx를 judge 비의존 정렬 프록시로 사용하고, DPO와 formatmix, GRPO 보정이 형식 통과율과 형식 통과 출력의 중앙값 NLL에 미치는 절충을 두 추천 카드 설정에서 정리한다."
...
- 구조화 UI 문구 생성에서 형식 admissibility와 언어 품질 프록시를 분리해 평가하는 문제 설정을 정의했다.
- 참조 언어모델 NLL을 NLL<sub>nat</sub>와 NLL<sub>ctx</sub>로 나누어 자동 선호 쌍 구성에 사용하는 절차를 구성했다.
- 3 문구 기본 카드와 메타 슬롯 확장 카드에서 구조 제약 강도에 따라 `formatmix`와 post-DPO GRPO 보정의 역할이 달라짐을 비교했다.
- strict-valid 부분집합에서 NLL 프록시와 LLM judge를 보조 인간 평가와 비교하고, 참조 언어모델 교체에 따른 NLL 점수 민감도를 점검했다.
...
- 형식 검증은 UI 배치 가능성을 가르는 이진 제약으로 두고, NLL<sub>nat</sub>와 NLL<sub>ctx</sub>는 자동 선호 쌍 구성을 위한 judge 비의존 연속 신호로 사용했다.
...
`spmi`는 입력 덕분에 출력이 얼마나 덜 상투적으로 설명되는지 보는 보조 진단값이다. 주요 pair construction과 정량 보고는 NLL<sub>nat</sub>와 NLL<sub>ctx</sub>를 중심으로 한다. 실험 로그의 원 필드명은 각각 `nllnat`, `nllctx`이며, 조건부 점수 계산에서는 입력 토큰을 loss에서 마스킹하고 출력 토큰만 기여하게 했다.
...
본문의 핵심 정량 표는 validation에서 선택한 고정 설정을 held-out test에서 다시 집계한 결과다. Appendix의 스윕은 대표 설정 선택과 민감도 해석을 보조하는 validation-stage 분석으로 구성된다. 두 과제의 split, 대표 pair 규모, 대표 형식 제어 방식은 Table 2에 요약한다.
...
대표 설정은 validation-stage 분석에서 먼저 고정한 뒤 held-out test에서 다시 집계했다. 선택 기준은 형식 통과율의 회복과 형식 통과 출력의 중앙값 NLL<sub>nat</sub>/NLL<sub>ctx</sub> 유지 사이의 절충이다. 3 문구 기본 카드에서는 먼저 DPO 기준선과 형식 인지 합성 pair의 teacher를 고정한 뒤 `formatmix` 비율을 스윕했다. 본문에서 사용하는 `formatmix 0.25`는 형식 통과율을 최대화한 설정이 아니라, DPO가 만든 낮은 NLL 구간을 유지하면서 형식 손실을 상당 부분 복구한 대표 지점이다.

메타 슬롯 확장 카드에서는 DPO 기준선, 형식 전용 DPO 대조군, 여러 GRPO 보정 변형을 validation에서 비교했다. 본문 대표 GRPO 변형은 최고 형식 통과율 설정이 아니라, 비교한 GRPO 변형 중 낮은 NLL<sub>nat</sub>를 유지하면서 형식 통과율을 크게 회복한 절충점이다. 세부 선택 근거와 후보별 수치는 Appendix Table 4와 Appendix Table 5에 정리한다.
...
<figure class="media-figure media-figure--wide-visual">
  ...
  <figcaption><strong>Figure 3.</strong> Held-out test 대표 결과의 형식-NLL 절충이다. A는 출력 단독 지표인 NLL<sub>nat</sub>, B는 입력 조건부 지표인 NLL<sub>ctx</sub>를 사용한다.</figcaption>
</figure>

3 문구 기본 카드에서는 `formatmix 0.25`가 품질 보존형 절충점이었다. DPO 기준선 대비 형식 통과율을 `0.628 -> 0.760`으로 회복하면서, 중앙값 NLL<sub>nat</sub>는 그대로 `3.096`을 유지했다. NLL<sub>ctx</sub>는 `2.715 -> 2.738`로 소폭 되돌아갔다.
...
<figure class="table-figure table-figure--comparison table-figure--compact-metrics">
  ...
  <figcaption><strong>Table 4.</strong> 3 문구 기본 카드의 held-out test formatmix 비교다. `0.25`는 형식 통과율을 의미 있게 회복하면서 DPO의 NLL<sub>nat</sub> 구간을 유지한 대표 지점이다. `0.50`은 형식 통과율은 더 높지만 NLL 회귀도 함께 커진다.</figcaption>
</figure>
...
대표 GRPO 보정은 이보다 나은 절충을 만들었다. DPO 대비 형식 통과율을 `0.376 -> 0.623`으로 올렸고, 중앙값 NLL<sub>nat</sub> 증가는 `0.140`에 머물렀다. NLL<sub>ctx</sub>는 오히려 `2.621 -> 2.594`로 낮아졌다.
...
절충을 완화하는 방식은 구조 제약 강도에 따라 달랐다. 3 문구 기본 카드에서는 `formatmix 0.25`가 DPO의 낮은 NLL<sub>nat</sub> 구간을 유지하면서 형식 통과율을 회복했다. 메타 슬롯 확장 카드에서는 형식 전용 DPO가 NLL을 크게 악화시켰고, 대표 GRPO 보정이 더 나은 형식-NLL 절충을 보였다. 다음 비교 축은 inference-time constraint, 다중 annotator 인간 평가, 참조 언어모델 교체, 다른 모델 규모에서의 반복 평가다.
...
대표 설정 선택은 validation-stage 스윕에서 수행했다. 3 문구 기본 카드에서는 형식 인지 합성 pair의 teacher를 먼저 고정하고, 이후 `formatmix` 비율을 비교했다.
    ...
    <table class="comparison-table metrics-table metrics-table--numeric-columns">
      ...
      <tbody>
        <tr>
          <td>Teacher 선택</td>
          <td>GPT-OSS-120B</td>
          <td class="align-right"><code>0.933</code></td>
          <td class="align-right"><code>3.031</code></td>
          <td class="align-right"><code>2.697</code></td>
          <td>형식 통과율은 낮지만 NLL 손실이 작아 후속 스윕의 기준 teacher로 사용</td>
        </tr>
        <tr>
          <td>Teacher 선택</td>
          <td>Gemini 2.5 Flash</td>
          <td class="align-right"><code>0.947</code></td>
          <td class="align-right"><code>3.119</code></td>
          <td class="align-right"><code>2.770</code></td>
          <td>형식 통과율은 높지만 NLL 회귀가 더 큼</td>
        </tr>
        <tr>
          <td>formatmix 비율</td>
          <td><code>0.20</code></td>
          <td class="align-right"><code>0.809</code></td>
          <td class="align-right"><code>2.898</code></td>
          <td class="align-right"><code>2.592</code></td>
          <td>NLL은 양호하지만 형식 회복이 상대적으로 작음</td>
        </tr>
        <tr>
          <td>formatmix 비율</td>
          <td><code>0.25</code></td>
          <td class="align-right"><code>0.836</code></td>
          <td class="align-right"><code>2.879</code></td>
          <td class="align-right"><code>2.590</code></td>
          <td>본문 대표 설정. 형식 회복과 NLL 유지의 절충이 가장 안정적</td>
        </tr>
        <tr>
          <td>formatmix 비율</td>
          <td><code>0.30</code></td>
          <td class="align-right"><code>0.845</code></td>
          <td class="align-right"><code>2.908</code></td>
          <td class="align-right"><code>2.602</code></td>
          <td>형식 통과율은 높지만 NLL 회귀가 증가</td>
        </tr>
        <tr>
          <td>formatmix 비율</td>
          ...
          <td class="align-right"><code>3.031</code></td>
          <td class="align-right"><code>2.697</code></td>
          <td>형식 우선 대안. 형식 회복은 크지만 NLL 회귀도 큼</td>
        </tr>
      </tbody>
    </table>
  </div>
  <figcaption><strong>Appendix Table 4.</strong> 3 문구 기본 카드의 teacher 및 formatmix 선택 근거다. 값은 validation-stage 분석이며, 본문 대표 설정은 형식 통과율 최대값이 아니라 품질 보존형 절충점으로 선택했다.</figcaption>
</figure>

메타 슬롯 확장 카드에서는 더 강한 구조 제약 아래에서 DPO, 형식 전용 DPO, GRPO 계열 변형을 함께 비교했다.
    ...
    <table class="comparison-table metrics-table metrics-table--numeric-columns">
      ...
      <tbody>
        <tr>
          ...
          <td class="align-right"><code>2.627</code></td>
          <td class="align-right"><code>2.431</code></td>
          <td>NLL은 낮지만 형식 회복 전 기준점</td>
        </tr>
        <tr>
          <td>형식 전용 DPO 대조군</td>
          <td class="align-right"><code>0.734</code></td>
          <td class="align-right"><code>3.240</code></td>
          <td class="align-right"><code>2.830</code></td>
          <td>형식 통과율은 회복하지만 NLL 손실이 큼</td>
        </tr>
        <tr>
          <td>균형형 GRPO 변형</td>
          <td class="align-right"><code>0.679</code></td>
          <td class="align-right"><code>2.822</code></td>
          <td class="align-right"><code>2.424</code></td>
          <td>NLL 손실은 작지만 형식 회복이 작음</td>
        </tr>
        <tr>
          <td>대표 GRPO 변형</td>
          <td class="align-right"><code>0.773</code></td>
          <td class="align-right"><code>2.811</code></td>
          <td class="align-right"><code>2.439</code></td>
          <td>본문 대표 설정. GRPO 계열 중 낮은 NLL<sub>nat</sub>와 큰 형식 회복을 함께 보임</td>
        </tr>
        <tr>
          <td>고형식 GRPO 변형 A</td>
          <td class="align-right"><code>0.793</code></td>
          <td class="align-right"><code>2.865</code></td>
          <td class="align-right"><code>2.438</code></td>
          <td>형식 통과율은 더 높지만 NLL<sub>nat</sub> 회귀가 증가</td>
        </tr>
        <tr>
          <td>고형식 GRPO 변형 B</td>
          <td class="align-right"><code>0.824</code></td>
          <td class="align-right"><code>2.885</code></td>
          <td class="align-right"><code>2.428</code></td>
          <td>최고 형식 통과율 설정이지만 NLL<sub>nat</sub> 회귀가 가장 큼</td>
        </tr>
      </tbody>
    </table>
  </div>
  <figcaption><strong>Appendix Table 5.</strong> 메타 슬롯 확장 카드의 대표 GRPO 선택 근거다. 값은 validation-stage 분석이며, 대표 GRPO 변형은 최고 형식 통과율이 아니라 형식 회복과 NLL 유지의 절충점으로 선택했다.</figcaption>
</figure>

보조 인간 평가와 judge 비교는 같은 strict-valid 샘플 집합 위에서 수행했다.
  ...
  <div class="table-shell">
    <table class="comparison-table metrics-table">
      ...
      <tbody>
        <tr>
          <td>샘플 구성</td>
          <td>NAT/CTX 4개 셀, 과제/평가축별 500개 샘플링, strict-valid 유효 행 수 총 <code>1,997</code></td>
        </tr>
        <tr>
          <td>평가자</td>
          <td>단일 annotator</td>
        ...
        <tr>
          <td>Blind 절차</td>
          <td>모델명과 실험 설정을 가리고, annotator는 출력 내용과 필요한 입력 정보만 보고 rubric 판단 수행</td>
        </tr>
        <tr>
          <td>평가 축</td>
          <td>NAT는 출력만 보고 자연스러움 평가, CTX는 입력과 출력을 함께 보고 문맥 적합성 평가</td>
        </tr>
        <tr>
          <td>척도</td>
          <td>두 축 모두 <code>0/1/2</code>의 3점 anchored rubric</td>
        </tr>
        <tr>
          <td>Subset 규칙</td>
          <td>인간 평가 및 judge 비교는 형식 검증을 이미 통과한 후보 집합에서만 수행</td>
        </tr>
        <tr>
          <td>Agreement</td>
          <td>단일 annotator 설계이므로 inter-annotator agreement는 보고하지 않음</td>
        </tr>
      </tbody>
    </table>
  </div>
  <figcaption><strong>Appendix Table 6.</strong> 보조 인간 평가와 judge 비교 프로토콜 요약이다. 이 표는 Table 7의 비교 범위를 해석하기 위한 평가 설계 정보다.</figcaption>
</figure>
...
<figure class="table-figure table-figure--comparison table-figure--compact-metrics">
  ...
  <figcaption><strong>Appendix Table 7.</strong> Polyglot-Ko-1.3B와 Qwen2.5 기반 NLL scorer의 셀별 상관이다. 값은 보조 인간 평가에 사용한 동일 샘플 집합에서 계산했다.</figcaption>
</figure>
