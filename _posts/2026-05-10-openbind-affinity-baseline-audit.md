...
last_modified_at: 2026-05-14 21:13:08 +0900
categories: ["BIO ML"]
...
excerpt: "OpenBind EV-A71 2A affinity release에서 raw pKD 상관과 MW+cLogP residual signal을 분리해 공개 prediction files와 동일 campaign supervised ligand-only control을 비교한 점검 노트."
description: "OpenBind EV-A71 2A compound-level affinity release에서 MW/cLogP/series progression confounding을 공개 자료 기준으로 수치화하고, raw pKD correlation 해석의 한계를 정리한 점검 노트."
...
image_alt: "OpenBind EV-A71 2A affinity 점검에서 raw Spearman과 MW+cLogP residual Spearman을 method별로 비교한 대표 차트"
hero_image: /assets/images/posts/openbind-affinity-baseline-audit/raw-vs-residual-spearman.svg
hero_alt: "OpenBind EV-A71 2A compound-level 점검에서 published prediction files와 ligand-only controls의 raw pKD Spearman 및 MW+cLogP residual Spearman을 비교한 차트"
hero_caption: "<strong>Figure 1.</strong> OpenBind EV-A71 2A compound-level affinity 점검에서 method별 raw pKD Spearman과 MW+cLogP residual Spearman을 나란히 비교했다. 왼쪽 group label은 동일 campaign supervised ligand-only controls, property baselines, published prediction files를 구분한다. 막대의 정확한 수치는 Table 3과 Appendix Table 1에 정리했다."
hero_frame: true
hero_compact: true
---

OpenBind의 첫 공개 release는 EV-A71 2A protease를 대상으로 structure, affinity measurement, reference benchmark를 함께 제공한다 <a class="citation-ref" href="#ref-openbind-first-release" aria-label="Reference 1">[1]</a>. 이 글은 그중 affinity prediction 결과를 compound-level retrospective analysis로 다시 읽는다. 목적은 새로운 confounding을 발견하는 것이 아니라, ligand size, lipophilicity, series progression이 raw affinity ranking에 섞일 수 있다는 기본 점검을 이 공개 release에서 실제 수치로 고정하는 데 있다.

Affinity campaign에서는 compound가 커지거나 더 lipophilic해지는 방향의 series progression이 potency 증가와 함께 나타날 수 있다. 이 경우 prediction score가 pKD와 상관을 보인다는 사실만으로는 target pocket이나 pose interaction에서 온 신호인지, MW/cLogP 같은 ligand property와 close analog progression을 따라간 신호인지 분리하기 어렵다. 따라서 raw pKD correlation은 먼저 ligand-property 기준선과 분리해서 읽어야 한다.
...
이 결과는 OpenBind 전체나 structure-based affinity prediction 일반에 대한 판정이 아니다. MW/cLogP confounding 자체가 뜻밖의 발견이라는 주장도 아니다. OpenBind release의 장점은 이런 기본 점검을 공개 자료 위에서 수치화할 수 있을 만큼 데이터와 benchmark가 정리되어 있다는 데 있다.

> **pKD**는 binding affinity를 로그 스케일로 표현한 값이다. 이 글에서는 affinity가 높은 compound가 더 높은 pKD를 갖는 ranking 문제로 해석한다.
>
> **MW+cLogP residual target**은 pKD를 molecular weight와 cLogP로 설명한 뒤, 그 선형 trend로 설명되지 않고 남은 부분을 뜻한다. 이는 structure-aware affinity signal의 정답이 아니라, ligand-property confounding을 점검하기 위한 진단용 비교 대상이다.
...
- OpenBind EV-A71 2A affinity release를 compound-level retrospective analysis로 점검했다. 핵심은 새로운 confounding의 발견이 아니라, MW/cLogP/series progression confounding을 공개 release에서 수치화한 기본 점검이다.
...
이 글은 sensorgram fitting이나 row-level affinity aggregation을 다시 모델링하지 않는다. 대신 OpenBind benchmark의 prediction files가 비교되는 compound-level 단위로 맞춘다. 즉, 같은 compound SMILES와 method 조합에 대해 공개 prediction score와 compound-level pKD를 놓고, raw affinity ranking과 ligand-property residual ranking을 나눠 본다.

이 단위 선택은 해석 범위를 좁힌다. 여기서 보는 것은 prospective discovery split의 성능이나 새 affinity model의 일반화 성능이 아니다. 공개 release 안에서 이미 제공된 prediction files를 해석할 때, raw pKD correlation이 어느 정도 ligand-property trend에 기대고 있는지 확인하는 후향적 점검이다.

Table 1은 이 글에서 사용한 compound-level 비교 단위를 요약한다.
...
<figure class="table-figure table-figure--comparison table-figure--compact-metrics">
  <div class="table-shell">
    <table class="comparison-table metrics-table metrics-table--compact-two-col">
      ...
      <tbody>
        ...
        <tr><td>public compound prediction rows</td><td class="align-right"><code>3458</code></td></tr>
  ...
  <figcaption><strong>Table 1.</strong> 이 글의 compound-level 비교 단위다. Row-level affinity measurement를 직접 모델링하지 않고, 공개 compound-level prediction table과 같은 method와 SMILES 조합 단위에서 점검했다.</figcaption>
</figure>

## Ligand-property 기준선

Raw pKD에서는 molecular weight 자체가 Spearman `0.484`를 보였다. MW+cLogP descriptive model도 pKD variance `R2 = 0.299`를 설명했다. Simple ligand-property baseline을 확인해야 한다는 점은 affinity benchmark 해석의 기본 전제다. 이 글에서 수치화한 부분은 그 전제가 OpenBind EV-A71 2A compound-level release 안에서 raw pKD correlation을 해석할 때 어느 정도의 크기로 작동하는지다.
...
이 비교에는 서로 다른 역할의 score가 함께 들어간다. Published prediction files는 OpenBind benchmark에 공개된 method score이고, `cv_ecfp_ridge`는 같은 campaign 안에서 affinity label을 사용해 학습한 supervised ligand-only control이다. 따라서 `cv_ecfp_ridge`는 published prediction files의 직접 경쟁자나 외부 제출 모델이 아니다.

이 control의 역할은 published score를 대체하는 것이 아니라, **같은 campaign label을 본 ligand-only model이 residual SAR를 어느 정도까지 잡을 수 있는지**를 보여주는 기준선이다. 만약 published score가 raw pKD에서는 상관을 보이지만 이 기준선보다 residual target에서 약하게 남는다면, raw correlation을 structure-aware affinity signal로 해석할 때 더 강한 caveat가 필요하다.
...
<figure class="table-figure table-figure--comparison table-figure--compact-metrics">
  ...
  <figcaption><strong>Table 3.</strong> MW+cLogP residual target에서의 주 비교다. Delta는 best published residual Spearman에서 best supervised ligand-only residual Spearman을 뺀 값이며, 반올림 전 값 기준 <code>-0.334</code>였다.</figcaption>
</figure>
...
<figure class="table-figure table-figure--comparison table-figure--metrics">
  ...
  <figcaption><strong>Table 4.</strong> Residual Spearman bootstrap interval이다. Best published residual signal은 약하게 양수로 남지만, 이 비교에서는 supervised ligand-only controls와 같은 범위로 올라오지는 않았다.</figcaption>
</figure>
...
<figure class="media-figure">
  <img src="/assets/images/posts/openbind-affinity-baseline-audit/butina-grouped-residual-spearman.svg" alt="Butina Tanimoto 0.6 cluster grouping에서 method별 grouped residual Spearman 평균과 bootstrap interval을 비교한 막대 차트">
  <figcaption><strong>Figure 3.</strong> Butina Tanimoto 0.6 cluster 기준 grouped residual Spearman sensitivity다. Figure 1과 같은 role grouping으로 배치했고, 막대는 bootstrap mean, 얇은 선은 95% interval이다. 결론은 뒤집히지 않았지만, grouping 자체는 singleton-heavy라 prospective scaffold split이 아니라 민감도 점검으로 해석한다.</figcaption>
</figure>
...
이 글에서 가장 안정적으로 남는 결론은 raw pKD correlation만으로 published score의 structure-aware affinity signal을 주장하기 어렵다는 점이다. 이는 새로운 발견이라기보다 ligand-property와 series progression이 섞인 campaign에서 먼저 해야 하는 기본 점검에 가깝다. OpenBind EV-A71 2A release는 이 점검을 공개 자료 위에서 수치화할 수 있게 해준다는 점에서 유용하다. Ligand-property baseline, supervised ligand-only control, grouped sensitivity를 함께 두면 published prediction files의 raw signal과 residual signal을 분리해 읽을 수 있다.

이번 공개 자료에 대한 결과는 제한적인 negative result로 정리한다. 그렇게 보는 이유는 published affinity prediction files가 MW+cLogP residual target에서 supervised ligand-only control만큼 남지 않았기 때문이다. 유용한 지점은 OpenBind release를 이용해 affinity benchmark를 해석할 때 어떤 baseline과 caveat를 먼저 둬야 하는지 공개 자료 기준의 점검 절차로 남긴다는 데 있다.
...
- 이 결과는 EV-A71 2A compound-level retrospective analysis에 한정되며, OpenBind 전체나 structure-based affinity prediction 일반을 판정하지 않는다.
- `cv_ecfp_ridge`는 published prediction files의 직접 경쟁자가 아니라, 동일 campaign supervised ligand-only residual SAR control이다.
- MW+cLogP residualization은 진단용 절차다. Medicinal chemistry campaign에서는 property trend와 series progression 자체도 실제 SAR와 얽힐 수 있으므로, residual target을 구조 정보의 정답으로 해석하지 않는다.
...
<figure class="table-figure table-figure--comparison table-figure--metrics">
  ...
  <figcaption><strong>Appendix Table 1.</strong> Method-level raw Spearman과 MW+cLogP residual Spearman이다. Delta는 <code>cv_ecfp_ridge</code> residual Spearman의 반올림 전 값을 기준으로 계산했으며, 직접 순위표가 아니라 residual comparison의 보조 표로 읽는다.</figcaption>
</figure>
