...
description: "다국어 상품 검색 임베딩에서 sparse click-label 평가의 한계를 LLM-judged 보조 평가로 점검하고, base model 교체, effective in-batch negatives, Matryoshka fine-tuning, bootstrap CI, 통합 후보 풀 audit, dim latency 실측을 함께 정리한다."
...
평가 질문은 세 가지다. 첫째, click-label MAP@10은 라벨 노이즈가 있어도 어떤 범위에서 모델 간 상대 비교 신호로 쓸 수 있는가. 둘째, 어떤 학습 변경이 다국어 상품 검색 임베딩에서 반복적으로 남는 개선을 만드는가. 셋째, Matryoshka fine-tuning은 768차원 품질을 유지하면서 256차원 임베딩을 저차원 색인 후보로 만들 수 있는가 <a class="citation-ref" href="#ref-mrl" aria-label="Reference 3">[3]</a>.

현 평가 조건에서 반복적으로 남은 개선 신호는 base model 교체와 batch/loss 변경을 통한 effective in-batch negative 규모 확대 쪽에 집중됐다. 데이터 합성, hard negative mining, weighted resampling, 단순 step 연장은 같은 데이터 분포 안에서는 안정적인 추가 개선을 만들지 못했다. Matryoshka fine-tuning은 dim 768 click-label 성능을 거의 유지하면서, LLM-judged 모델별 후보 풀(self-pool) 기준 dim 256 MAP@10을 dim 768과 0.002 이내로 맞췄다. 이후 query-level paired bootstrap은 `0.0005-0.0009` 수준의 overall 차이가 측정 한계 안에 있음을 확인했고, 통합 후보 풀(union-pool) audit은 dim 768이 dim 256보다 약 `0.0049` 높은 보정 단서를 남겼다.
...
- title prompt와 longer training을 포함한 레시피 개선 뒤, CachedMultipleNegativesRankingLoss와 per-device batch `192` 조합은 MAP@10을 `0.8082 -> 0.8151`로 올렸다. Paired bootstrap에서도 이 양성 대조 비교는 `+0.0068`, 95% CI `[+0.0035, +0.0101]`로 0과 분리됐다.
- Matryoshka fine-tuning은 dim 768 click-label MAP@10을 `0.8151 -> 0.8146`으로 거의 유지했다. LLM-judged 모델별 후보 풀(self-pool) 기준 dim 256 MAP@10은 non-MRL `0.9321`에서 `0.9506`으로 회복했다.
...
- 보강 검증은 세 결론을 분리했다. 작은 click-label delta는 query-level bootstrap에서 대부분 측정 한계 안에 있었고, LLM-judged 모델별 후보 풀(self-pool) 점수는 통합 후보 풀(union-pool)보다 높게 잡히지만 모델 개선 방향은 유지됐다. dim 256은 약 `0.005` MAP@10을 1/3 인덱스 메모리와 검색 QPS 개선과 교환하는 선택지다.
- 같은 데이터 분포 안에서 추가 개선의 병목은 학습 설정 조정보다 semantic intent 데이터와 카탈로그 전체 검증 쪽으로 이동했다.
...
이 평가는 noisy하다. 한 query의 positive 수가 평균적으로 적고, 특히 `립`, `선크림`처럼 넓은 검색어에는 클릭되지 않았지만 관련 있는 상품이 많다. 그래서 click-label MAP@10의 절대값은 실제 사용자가 보는 관련성보다 낮을 수 있다. 학습 전후처럼 효과 크기가 큰 비교에서는 상대 개선 방향이 LLM-judged 평가와 함께 움직였다. 근접 run 차이는 query-level paired bootstrap으로 다시 점검했다. 전체 `10,934`개 test `(query, lang)` 그룹 기준 overall MAP@10의 95% 측정 한계는 약 `±0.003`이었다.
...
모든 주요 비교는 동일한 v1 데이터셋, same-language retrieval corpus, click-label 평가 조건을 기준으로 해석한다. 이 보고서의 주요 수치는 단일 run 계보에서 나온 결과이지만, 근접한 MAP@10 차이는 query-level paired bootstrap으로 사후 점검했다. 표에서 `측정 노이즈 수준`이라고 부르는 작은 overall 차이는 대체로 95% CI가 0을 포함했다. 실험 계보는 크게 네 단계다.
...
모델 계보를 보면 개선 폭은 초반에 크고 뒤로 갈수록 작아졌다. multilingual-e5-base 기준선의 최고치는 `0.7732`였고, EmbeddingGemma-300M으로 바꾸면 `0.7953`이 됐다. 이후 title prompt, step 수, CachedMNRL, batch size를 묶은 레시피 개선으로 `0.8151`까지 올라갔다. 이 bundle 안에서 별도로 비교된 batch/loss 변경은 `0.8082 -> 0.8151`의 `+0.0069` 신호로 남았다. 사후 paired bootstrap에서도 이 양성 대조 비교는 `+0.0068`, 95% CI `[+0.0035, +0.0101]`로 0을 배제했다. Matryoshka fine-tuning은 dim 768 기준 `0.8146`으로 거의 같은 수준을 유지했다.
...
가장 안정적으로 남은 개선은 두 축이었다. 첫째는 base model 교체다. e5-base에서 EmbeddingGemma-300M으로 바꾸면 KO/EN/JP 모두 개선됐다. 둘째는 recipe bundle 안에서 확인된 batch/loss 변경 신호다. CachedMultipleNegativesRankingLoss와 batch 192 조합은 batch 48 MNRL 대비 `+0.0069`를 만들었고, paired bootstrap에서도 구분 가능한 개선으로 남았다. 이는 effective in-batch negatives 확대와 일관된다.
...
<figure class="table-figure table-figure--comparison">
  ...
  <figcaption><strong>Table 5.</strong> 주요 lever의 역할 기반 요약이다. delta는 각 실험의 직접 비교 기준 대비 값이며, 동일한 평가 조건에서 해석한다. 사후 paired bootstrap에서는 overall 측정 한계가 약 ±0.003으로 추정됐고, batch/loss 변경 양성 대조 비교는 0과 분리됐다.</figcaption>
</figure>

이 결과는 추가 개선 신호가 약했던 비교군도 해석상 중요하다는 점을 보여준다. 현재 데이터 분포에서는 lexical overlap이 강한 query/document pair가 많았고, 그런 영역은 이미 상위 retrieval이 강했다. Bootstrap 결과까지 함께 보면, overall `+0.001` 안팎의 변화는 현 test set에서 구분하기 어렵다. 추가 개선은 같은 pair를 더 세게 학습하는 것보다, 의미 의도 query나 카탈로그 전체 후보군처럼 평가 난이도를 바꾸는 쪽에서 나올 가능성이 크다.
...
중요한 점은 개선 방향이다. 학습 전후 개선 폭은 click-label과 LLM-judged에서 모두 컸다. click-label 전체 MAP@10은 `0.6558 -> 0.8146`, LLM-judged MAP@10은 `0.7739 -> 0.9525`였다. 따라서 click-label은 절대값을 낮게 보일 수 있지만, 효과 크기가 큰 비교에서는 같은 방향의 상대 개선 신호로 쓸 수 있었다. 작은 delta의 모델 순위는 paired bootstrap과 LLM 보조 평가를 함께 보아야 한다.
...
<figure class="table-figure table-figure--comparison">
  ...
  <figcaption><strong>Table 6.</strong> click-label과 LLM-judged 평가의 역할 차이다. click-label은 전체 실험 비교의 기본 지표이고, 표의 LLM-judged 수치는 모델별 top-30 후보 풀(self-pool) 기준에서 sparse positive 때문에 낮아진 절대 점수를 보정해 해석하는 보조 지표다.</figcaption>
</figure>
...
<figure class="media-figure media-figure--wide-visual">
  ...
  <figcaption><strong>Figure 2.</strong> Matryoshka 차원별 LLM-judged 모델별 후보 풀(self-pool) MAP@10이다. non-MRL 모델은 dim 256과 dim 128에서 품질 손실이 커졌지만, MRL fine-tuning은 self-pool 기준 dim 256을 dim 768과 거의 같은 수준으로 유지했다.</figcaption>
</figure>
...
<figure class="table-figure table-figure--comparison">
  ...
  <figcaption><strong>Table 7.</strong> Matryoshka 차원별 모델별 후보 풀(self-pool) 품질이다. top-1 변경은 dim 768 결과와 같은 query에서 1위 상품이 바뀐 횟수다. dim 256은 top-1 상품이 자주 바뀌지만 self-pool relevance 기준 MAP@10과 Hit@1은 거의 유지됐다.</figcaption>
</figure>

현재 same-language corpus와 KO 100개 LLM-judged audit의 retrieval quality 기준으로는 dim 256이 유력한 저차원 후보였다. Self-pool MAP@10은 `0.9506`으로 dim 768의 `0.9525`와 0.002 이내였고, Hit@1은 둘 다 `1.0000`이었다. 그러나 통합 후보 풀(union-pooled) audit에서는 dim 768 MAP@10이 `0.9430`, dim 256이 `0.9381`로 벌어져 약 `0.0049`의 품질 차이가 남았다. Dim 256은 union-pool에서도 non-MRL 기준 모델의 dim 768보다 높게 남았으므로, 저차원 후보라는 판단 자체는 유지된다.

Latency 실측은 이 절충의 비용 쪽을 확인했다. 모델은 항상 768차원 embedding을 출력하므로 encode 비용은 줄지 않는다. 단일 query encode는 `47.6 ms`, batch encode는 `455 docs/s`로 차원 선택과 무관했다. 하지만 vector index 저장과 검색에서는 dim 256의 이득이 분명했다. 1M fp16 exact-search 기준 인덱스 메모리는 768차원 `1,536 MB`에서 256차원 `512 MB`로 줄었고, GPU 검색은 query당 `6.47 ms -> 2.23 ms`, QPS `8,070 -> 14,545`로 개선됐다. 같은 조건에서 dim 256은 dim 768 대비 메모리 `0.333`, 1M corpus GPU QPS `1.80x`였다.
...
세 번째 결론은 Matryoshka fine-tuning의 색인 차원 관점 의미다. MRL은 dim 768 ranking을 크게 올리는 기술이 아니었다. 그러나 fine-tuning 과정에서 약화될 수 있는 저차원 prefix 품질을 회복했고, dim 256을 비용 절감 후보로 만들었다. 통합 후보 풀 audit을 반영하면 dim 256은 dim 768과 완전히 같은 품질이 아니라 약 `0.005` MAP@10 낮은 후보로 보는 편이 정확하다. 대신 latency 실측에서는 1M exact-search 기준 인덱스 메모리 1/3, GPU QPS `1.80x`가 확인됐다. 따라서 dim 256의 의미는 무손실 압축이 아니라 retrieval 품질과 vector index 비용 사이의 명시적 절충이다.
...
- dim 256의 index 저장 비용과 exact-search latency 이득은 실측했지만, encode 비용은 줄지 않았다. ANN index, quantization, batch serving, 실제 vector DB 설정에서는 절감폭이 달라질 수 있다.
