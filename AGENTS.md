...
- 특정 논문에서 제안되어 글의 분석 대상이 되는 방법은, 방법론의 첫 실질적 설명에서 대표 저자 `et al.` 또는 기관명, 방법명, 인용을 함께 둔다. 이후에는 방법명과 인용 번호를 사용한다. 범용 기법이나 부수적으로 언급되는 출처에는 이를 강제하지 않는다.
- 목차는 시간 순서보다 증거의 역할을 기준으로 묶는다. 준비 조건이 둘 이상이면 `## Experimental Setup` 아래에, 연속된 측정 결과와 조건별 관찰이 둘 이상이면 `## Results` 아래에 둔다. 결과를 넘어 재사용 가능한 진단 기준이나 해석 틀은 별도 최상위 섹션으로 둔다.
